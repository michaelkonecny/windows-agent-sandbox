...
def _op_install(**kwargs) -> dict:
    ...
    from sbx.identity import (
        SANDBOX_GROUP, SANDBOX_USER, grant_runner_access, install_user,
        lock_system_dirs,
    )
    ...
    lock_system_dirs(winapi.account_sid(SANDBOX_GROUP))
...
def _op_uninstall_cleanup(**kwargs) -> dict:
    from sbx.identity import (
        SANDBOX_GROUP, SANDBOX_USER, revoke_runner_access, uninstall_user,
        unlock_system_dirs,
    )
    ...
    try:
        unlock_system_dirs(winapi.account_sid(SANDBOX_GROUP))
    except OSError:
